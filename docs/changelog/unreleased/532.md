...
- Fixed native scratch-sync commit throughput by caching immutable trust-anchor
  digests, using ordered trust-pin lookups, and reading VCT auxiliary windows
  and continuation locators from the already-audited committed engine
  projection instead of repeatedly walking the durable selected path
  ([#532](https://github.com/zakura-core/zakura/pull/532)).
