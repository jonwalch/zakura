...
fn config_load_defaults() {
    ...
    let config = ZakuradConfig::load(None).expect("Should load default config");
    ...
    assert_eq!(
        config.network.zakura.max_connections_per_ip,
...
"#;
    ...
    let config = ZakuradConfig::load(Some(config_path)).expect("load config from file");
...
fn config_nonexistent_file_errors() {
    ...
    ZakuradConfig::load(Some(nonexistent_path)).expect_err("Should fail to load nonexistent file");
}
...
fn config_env_override_defaults() {
    ...
    let config = ZakuradConfig::load(None).expect("load config with env vars");
...
"#;
    ...
    let config = ZakuradConfig::load(Some(config_path)).expect("load config");
...
"#;
    ...
    ZakuradConfig::load(Some(config_path)).expect_err("Should fail to load invalid TOML");
}
...
fn config_zakura_tracing_log_file_env() {
    ...
    let config = ZakuradConfig::load(None).expect("load config with ZAKURA_TRACING__LOG_FILE");
    assert_eq!(
...
fn config_zakura_mining_miner_address_from_toml() {
    ...
    let config = ZakuradConfig::load(Some(file.path().to_path_buf()))
