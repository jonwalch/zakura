...
impl Solution {
        ...
        F: FnMut() -> SolverAction,
    {
        ...
        while !is_shutting_down() {
            ...
            // The solver allocates its own state, and reports a failure to do so
            // rather than aborting. Treat it like a cancellation: the caller
            // requests a fresh template instead of spinning on the same one.
            let solve_result = match solve_result {
                Ok(solve_result) => solve_result,
                Err(error) => {
                    info!(?error, "equihash solver could not start a pass");
                    return Err(SolverCancelled);
                }
            };
