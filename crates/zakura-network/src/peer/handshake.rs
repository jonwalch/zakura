...
/// Returns true when a syncing node must reject an outbound peer that cannot serve blocks.
fn is_non_serving_outbound_peer(
    ...
    PeerTransport: AsyncRead + AsyncWrite + Unpin + Send + 'static,
{
    ...
    // provide historic blocks. Near the tip, accept them again because pruned peers can still
    // provide recent blocks and transactions. Inbound and isolated peers are unaffected.
    if is_non_serving_outbound_peer(connected_addr, remote.services, is_syncing) {
        debug!(
            remote_ip = %addr_label,
            ?remote.services,
            ?remote.user_agent,
