...
async fn get_block_data(
    ...
    let zakura_state::ReadResponse::SaplingTree(sapling_tree) = read_state
        .clone()
        .oneshot(zakura_state::ReadRequest::SaplingTree(
            HashOrHeight::Height((height as u32).try_into().unwrap()),
        ))
        .await
    ...
    let zakura_state::ReadResponse::BlockInfo(block_info) = read_state
        .clone()
        .oneshot(zakura_state::ReadRequest::BlockInfo(HashOrHeight::Height(
...
async fn rpc_getblockheader() {
    ...
    for (i, block) in blocks.iter().enumerate() {
        ...
        let zakura_state::ReadResponse::SaplingTree(sapling_tree) = read_state
            .clone()
            .oneshot(zakura_state::ReadRequest::SaplingTree(height.into()))
            .await
            .expect("should have sapling tree for block hash")
        else {
            panic!("unexpected response to SaplingTree request")
...
async fn rpc_getrawtransaction() {
    ...
    let mut mempool: MockService<_, _, _, BoxError> = MockService::build().for_unit_tests();
    let (state, read_state, _, _) = zakura_state::populated_state(blocks.clone(), &Mainnet).await;
    ...
    let run_state_test_case = |block_idx: usize, block: Arc<Block>, tx: Arc<Transaction>| {
        ...
        async move {
            ...
            let depth_response = read_state
                .oneshot(zakura_state::ReadRequest::Depth(block.hash()))
                .await
                .expect("state request should succeed");

            let zakura_state::ReadResponse::Depth(depth) = depth_response else {
...
async fn getaddresstxids_single_equals_object_full_range() {
    ...
    let (_, read_state, tip, _) = zakura_state::populated_state(blocks.clone(), &network).await;
...
async fn rpc_getpeerinfo() {
    ...
    let outbound_mock_peer_address = zakura_network::types::MetaAddr::new_connected(
    ...
    let inbound_mock_peer_address = zakura_network::types::MetaAddr::new_connected(
        std::net::SocketAddr::new(
            std::net::IpAddr::V4(std::net::Ipv4Addr::new(127, 0, 0, 1)),
            44444,
        )
    ...
    let not_connected_mock_peer_adderess = zakura_network::types::MetaAddr::new_initial_peer(
        std::net::SocketAddr::new(
            std::net::IpAddr::V4(std::net::Ipv4Addr::new(127, 0, 0, 1)),
            55555,
        )
...
async fn rpc_submitblock_errors() {
    ...
    let mut mempool: MockService<_, _, _, BoxError> = MockService::build().for_unit_tests();
    // Create a populated state service
    let (state, read_state, tip, _) = zakura_state::populated_state(blocks, &Mainnet).await;
    ...
    for &block_bytes in zakura_test::vectors::CONTINUOUS_MAINNET_BLOCKS.values() {
    ...
    let submit_block_response = rpc
        .submit_block(
            HexData(zakura_test::vectors::BAD_BLOCK_MAINNET_202_BYTES.to_vec()),
            None,
        )
        .await;
...
/// aborting `zakurad`) when given a unified address whose Sapling receiver has a valid
...
async fn rpc_addnode() {
    ...
    let mock_address_book =
        zakura_network::address_book_peers::mock::SharedMockAddressBookPeers::default();
...
async fn rpc_gettxout() {
    ...
    let run_test_case = |_block_idx: usize, _block: Arc<Block>, tx: Arc<Transaction>| {
        ...
        async move {
            ...
            let depth_response = read_state
                .oneshot(zakura_state::ReadRequest::Depth(_block.hash()))
                .await
                .expect("state request should succeed");
