...
//! ```sh
//! cargo insta test --review --release -p zakura-rpc --lib -- test_rpc_response_data
//! ```
...
use zakura_state::{GetBlockTemplateChainInfo, ReadRequest, ReadResponse, MAX_ON_DISK_HEIGHT};
use zakura_test::{
...
async fn test_z_get_treestate() {
    ...
    let (state, read_state, tip, _) =
        zakura_state::populated_state(blocks.clone(), &custom_testnet).await;
...
async fn test_rpc_response_data_for_network(network: &Network) {
    ...
    let mut mempool: MockService<_, _, _, zakura_node_services::BoxError> = MockService::build()
    ...
    let (state, read_state, tip, _) = zakura_state::populated_state(blocks.clone(), network).await;
