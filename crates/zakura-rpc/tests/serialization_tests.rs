...
fn test_get_block_1() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let new_obj = GetBlockResponse::Object(Box::new(BlockObject::new(
        zakura_chain::block::Hash(hash),
        confirmations,
        size,
        height,
        version,
        merkle_root.map(zakura_chain::block::merkle::Root),
        block_commitments,
        final_sapling_root,
        final_orchard_root,
        n_tx,
        tx.iter()
            .map(|h| GetBlockTransaction::Hash(zakura_chain::transaction::Hash(*h)))
            .collect(),
        time,
        nonce,
        solution,
        bits.map(|d| {
            zakura_chain::work::difficulty::CompactDifficulty::from_bytes_in_display_order(&d)
...
"#;
    ...
    let new_obj = GetBlockHeightAndHashResponse::new(
        zakura_chain::block::Height(height),
        zakura_chain::block::Hash(hash),
    );
...
"#;
    ...
    let new_obj = GetSubtreesByIndexResponse::new(
        ...
        vec![SubtreeRpcData {
            ...
            end_height: zakura_chain::block::Height(subtree_end_height),
        }],
    );
        ...
        .map(|input| match input {
            ...
            Input::NonCoinbase {
                ...
                Input::NonCoinbase {
                    txid: txid.clone(),
                    vout: *vout,
                    script_sig: ScriptSig::new(asm, zakura_chain::transparent::Script::new(&hex)),
                    sequence: *sequence,
                    value: *value,
                    value_zat: *value_zat,
