...
fn worst_case_tx_verification(c: &mut Criterion) {
    ...
    let rayon_threads =
        thread_count_from_env("ZAKURA_BENCH_RAYON_THREADS", first_case.rayon_threads);
    let tokio_worker_threads = thread_count_from_env(
        "ZAKURA_BENCH_TOKIO_WORKER_THREADS",
        first_case.tokio_worker_threads,
    );

    init_rayon(rayon_threads);

    // The memo toggles are read once inside the verifier statics; echo them here so a
    // captured benchmark log is self-describing about which leg it measured.
    println!(
        "worst_case_tx_verification: rayon_threads={rayon_threads}; tokio_worker_threads={tokio_worker_threads}; halo2_memo_disabled={}; sapling_bench_memo_enabled={}",
        std::env::var("ZAKURA_BENCH_DISABLE_HALO2_MEMO").unwrap_or_else(|_| "0".to_string()),
        std::env::var("ZAKURA_BENCH_ENABLE_SAPLING_MEMO").unwrap_or_else(|_| "0".to_string()),
    );
...
/// Reads a thread count from `var`, falling back to the compiled-in case value.
///
/// Both pools are process-global and [`validate_benchmark_cases`] already requires one value
/// per process, so sweeping thread counts means one process per point. The default matches
/// the hardcoded case table, so an unset environment reproduces the original numbers.
fn thread_count_from_env(var: &str, default: usize) -> usize {
    ...
    assert!(parsed > 0, "{var} must be greater than zero");
