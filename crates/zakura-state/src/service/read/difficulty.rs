...
fn adjust_difficulty_and_time_for_testnet(
    ...
    // A minimum-difficulty block is only consensus-valid when its time is strictly past
    // `previous_block_time + 6 * PoWTargetSpacing`. Keep standard difficulty until the
    // current time reaches that boundary, rather than future-dating the template to obtain
    // minimum difficulty early.
    //
    // `cur_time` was already clamped into `[min_time, max_time]` by the caller. There is no
    // need to restore the unclamped time: clamping up makes minimum difficulty more likely,
    // while clamping down almost always still leaves a minimum-difficulty block.
...
mod tests {
    ...
    fn recent_block_data(network: &Network) -> Vec<(CompactDifficulty, DateTime<Utc>)> {
        ...
        (0..POW_ADJUSTMENT_BLOCK_SPAN)
            .map(|offset| {
                let offset =
                    u32::try_from(offset).expect("the difficulty adjustment window fits in u32");
                (
                    threshold,
                    DateTime32::from(PREVIOUS_BLOCK_TIME - offset).into(),
                )
            })
            .collect()
    }
    ...
    #[test]
    fn minimum_difficulty_starts_strictly_past_the_threshold() {
        let network = Network::new_default_testnet();
        let mut at_threshold = chain_info(PREVIOUS_BLOCK_TIME + MINIMUM_DIFFICULTY_GAP);
        let mut past_threshold = chain_info(PREVIOUS_BLOCK_TIME + MINIMUM_DIFFICULTY_GAP + 1);
