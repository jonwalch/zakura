...
impl DbFormatChange {
    ...
    pub fn check_new_blocks(db: &ZakuraDb) -> Self {
    ...
    pub fn format_validity_checks_quick(db: &ZakuraDb) -> Result<(), String> {
        let timer = CodeTimer::start();
    ...
    fn mark_as_newly_created(db: &ZakuraDb) {
        let running_version = db.format_version_in_code();
        let disk_version = db
            .format_version_on_disk()
    ...
    fn mark_as_upgraded_to(db: &ZakuraDb, format_upgrade_version: &Version) {
        let running_version = db.format_version_in_code();
        let disk_version = db
            .format_version_on_disk()
            .expect("unable to read database format version file")
    ...
    fn mark_as_downgraded(db: &ZakuraDb) {
        let running_version = db.format_version_in_code();
        let disk_version = db
            .format_version_on_disk()
            .expect("unable to read database format version file")
