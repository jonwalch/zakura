...
fn help_no_args() -> Result<()> {
    ...
    output.any_output_line(
        ...
        "stdout",
        "are valid zakurad semantic versions",
    )?;
...
fn version_no_args() -> Result<()> {
    ...
    output.output_check(
        ...
        &output.output.stdout,
        "stdout",
        "a valid zakurad semantic version",
    )?;
...
fn config_tests() -> Result<()> {
    valid_generated_config("start", "Starting zakurad")?;
...
/// Test that `zakurad` runs the start command with no args
#[tracing::instrument]
fn app_no_args() -> Result<()> {
    ...
    tracing::info!(
        ?testdir,
        "running zakurad with no config (default settings)"
    );
...
/// Test that `zakurad start` can parse the output from `zakurad generate`.
#[tracing::instrument]
fn valid_generated_config(command: &str, expect_stdout_line_contains: &str) -> Result<()> {
    ...
    output.assert_was_killed().wrap_err("Possible port or cache conflict. Are there other acceptance test, zakurad, or zcashd processes running?")?;
...
/// Check if the config produced by current zakurad is stored.
#[cfg(not(target_os = "windows"))]
#[tracing::instrument]
...
fn stored_configs_parsed_correctly() -> Result<()> {
    ...
    use zakurad::application::ZakuradApp;
        ...
        .flatten()
    {
        ...
        tracing::info!(
            ?config_file_path,
            "testing old config can be parsed by current zakurad"
        );
...
fn stored_configs_work() -> Result<()> {
        ...
        .flatten()
    {
        ...
        tracing::info!(
            ?stored_config_path,
            ?success_regexes,
            "waiting for zakurad to parse config and start logging"
        );
...
/// Test if `zakurad` can sync the first checkpoint on mainnet.
///
...
fn restart_stop_at_height_for_network(network: Network, height: block::Height) -> Result<()> {
    ...
    // if stopping corrupts the rocksdb database, zakurad might hang or crash here
...
/// Test if `zakurad` can activate the mempool on mainnet.
...
/// Test if `zakurad` can run side by side with the mempool.
...
fn create_cached_database(network: Network) -> Result<()> {
    ...
    create_cached_database_height(
        ...
        zakura_state::StorageMode::Archive,
...
/// Test if `zakurad` can fully sync the chain on testnet.
///
/// This test takes a long time to run, so we don't run it by default. This test is only executed
...
/// Test will start 2 zakurad nodes one after the other using the same Zcash listener.
...
/// Start 2 zakurad nodes using the same metrics listener port, but different
...
/// Start 2 zakurad nodes using the same RPC listener port, but different
/// state directories and Zcash listener ports. The first node should get
...
/// Start 2 zakurad nodes using the same state directory, but different Zcash
...
#[test]
fn zakura_state_conflict() -> Result<()> {
    let _init_guard = zakura_test::init();
    ...
    let contains = if cfg!(unix) {
        ...
        dir_conflict_full.push(format!(
            "v{}",
            zakura_state::state_database_format_version_in_code().major,
        ));
...
where
    T: ZakuradTestDirExt + std::fmt::Debug,
...
/// Test `zakura-checkpoints` on mainnet.
///
...
/// Test `zakura-checkpoints` on testnet.
...
/// and that restarting `zakurad` doesn't change the format version.
#[tokio::test]
...
/// and that restarting `zakurad` doesn't change the updated format version.
///
...
async fn state_format_test(
    ...
    let zakurad = spawn_zakurad_without_rpc(network.clone(), test_name, false, false, None, false)?;

    // Skip the test unless it has the required state and environmental variables.
    let Some(mut zakurad) = zakurad else {
        return Ok(());
    };
    ...
    if let Some(fake_version) = fake_version {
        ...
        tracing::info!(?network, "running {test_name} using zakura-state");

        let config = UseAnyState
            .zakurad_config(test_name, false, Some(dir.path()), network)
            .expect("already checked config")?;

        zakura_state::write_state_database_format_version_to_disk(
            &config.state,
            fake_version,
            network,
        )
    ...
    for reopened in 0..reopen_count {
        ...
        let mut zakurad =
            spawn_zakurad_without_rpc(network.clone(), test_name, false, false, dir, false)?
                .expect("unexpectedly missing required state or env vars");

        tracing::info!(?network, "running {test_name} using zakurad");

        if expect_older_version {
            zakurad.expect_stdout_line_matches("trying to open older database format")?;
            zakurad.expect_stdout_line_matches("marked database format as upgraded")?;
            zakurad.expect_stdout_line_matches("database is fully upgraded")?;
        } else if expect_newer_version {
            zakurad.expect_stdout_line_matches("trying to open newer database format")?;
...
async fn fully_synced_rpc_z_getsubtreesbyindex_snapshot_test() -> Result<()> {
    ...
    let (mut zakurad, zakura_rpc_address) = if let Some(zakurad_and_address) =
        ...
        tracing::info!("running fully synced zakurad z_getsubtreesbyindex RPC test");
    ...
    // Wait for zakurad to load the full cached blockchain.
    zakurad.expect_stdout_line_matches(SYNC_FINISHED_REGEX)?;

    // Create an http client
    let client =
        RpcRequestClient::new(zakura_rpc_address.expect("already checked that address is valid"));
...
async fn validate_regtest_genesis_block() {
    ...
    let state = zakura_state::init_test(&network).await;
    let (
        block_verifier_router,
        _transaction_verifier,
        _parameter_download_task_handle,
        _max_checkpoint_height,
    ) = zakura_consensus::router::init_test(zakura_consensus::Config::default(), &network, state)
        .await;
...
/// - the node runs in [`StorageMode::Pruned`](zakura_state::StorageMode::Pruned),
...
async fn pruned_storage_mode_prunes_during_regtest_sync() -> Result<()> {
    ...
    let summary = zakura_state::preview_prune_finalized_state(
        state_config,
        &network,
...
async fn trusted_chain_sync_handles_forks_correctly() -> Result<()> {
    ...
    use zakura_chain::{chain_tip::ChainTip, primitives::byte_array::increment_big_endian};
    use zakura_rpc::methods::GetBlockHashResponse;
    use zakura_state::{ReadResponse, Response};
    ...
    let (read_state, _latest_chain_tip, mut chain_tip_change, _sync_task) =
        zakura_rpc::sync::init_read_state_with_syncer(
            config.state,
            &config.network.network,
            indexer_listen_addr,
        )
    ...
    for expected_block in blocks.clone() {
        ...
        let ReadResponse::Block(read_state_block) = read_state
            .clone()
            .oneshot(zakura_state::ReadRequest::Block(height.into()))
            .await
            .map_err(|err| eyre!(err))?
        else {
    ...
    for mut block in next_blocks {
        let ReadResponse::ChainInfo(chain_info) = read_state2
            .clone()
            .oneshot(zakura_state::ReadRequest::ChainInfo)
            .await
            .map_err(|err| eyre!(err))?
        else {
        ...
        let Response::Committed(block_hash) = state2
            .clone()
            .oneshot(zakura_state::Request::CommitSemanticallyVerifiedBlock(
...
/// `cargo test --package zakura --test acceptance -- nu6_funding_streams_and_coinbase_balance --exact --show-output`
#[tokio::test(flavor = "multi_thread")]
async fn nu6_funding_streams_and_coinbase_balance() -> Result<()> {
    ...
    let (
        ...
        _parameter_download_task_handle,
        _max_checkpoint_height,
    ) = zakura_consensus::router::init_test(
    ...
    let zakura_state::GetBlockTemplateChainInfo {
        chain_history_root, ..
...
async fn has_spending_transaction_ids() -> Result<()> {
    use std::sync::Arc;
    use tower::Service;
    use zakura_chain::{chain_tip::ChainTip, transparent::Input};
    use zakura_state::{ReadRequest, ReadResponse, SemanticallyVerifiedBlock, Spend};
    ...
    let (mut state, mut read_state, latest_chain_tip, _chain_tip_change) =
        common::cached_state::start_state_service_with_cache_dir(&Mainnet, zakurad_state_path)
            .await?;

    tracing::info!("committing blocks to non-finalized state");

    for block in non_finalized_blocks {
        use zakura_state::{CommitSemanticallyVerifiedBlockRequest, MappedRequest};
...
async fn restores_non_finalized_state_and_commits_new_blocks() -> Result<()> {
    ...
    child.kill(true)?;
    // Wait for zakurad to fully terminate to ensure database lock is released.
    child
        .wait_with_output()
        .wrap_err("failed to wait for zakurad to fully terminate")?;
    ...
    child
        .wait_with_output()
        .wrap_err("failed to wait for zakurad process to exit after kill")?;
...
/// The second zakurad instance will connect to the first one, and when the first one mines
...
async fn disconnects_from_misbehaving_peers() -> Result<()> {
    ...
    // Get the zakurad config
    let mut config = test_type
        .zakurad_config(test_name, false, None, &network1)
        .expect("already checked config")?;
...
/// Verifies that zakurad and zcashd agree on block count and best block hash.
///
...
/// Sends a transparent transaction via zcashd and confirms it appears in zakurad's mempool.
///
...
/// Verifies that zcashd restarts after an unexpected exit without corrupting zakurad.
///
