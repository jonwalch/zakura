...
use zakura_consensus::{error::TransactionError, transaction, Config as ConsensusConfig};
use zakura_network::{
    constants::{
...
async fn peer_pushed_transaction_is_verified_without_redownload() -> Result<(), crate::BoxError> {
    ...
    let peer_source =
        zakura_network::PeerSource::LegacySocket(SocketAddr::from(([127, 0, 0, 1], 8233)).into());
...
async fn peer_mempool_full_queue_is_refused_without_disconnect() -> Result<(), crate::BoxError> {
    ...
    let peer_id = zakura_network::zakura::ZakuraPeerId::new(vec![7; 32])
        .expect("test peer id is within bounds");
    let source = zakura_network::PeerSource::Zakura(peer_id);
    let txid =
        |index: u8| UnminedTxId::from_legacy_id(zakura_chain::transaction::Hash([index; 32]));
...
async fn mempool_transaction_expiration() -> Result<(), crate::BoxError> {
    ...
    *tx1.expiry_height_mut() = zakura_chain::block::Height(3);
    ...
    let more_blocks: Vec<Arc<Block>> = vec![
        zakura_test::vectors::BLOCK_MAINNET_4_BYTES
            .zcash_deserialize_into()
            .unwrap(),
        zakura_test::vectors::BLOCK_MAINNET_5_BYTES
            .zcash_deserialize_into()
            .unwrap(),
        zakura_test::vectors::BLOCK_MAINNET_6_BYTES
            .zcash_deserialize_into()
            .unwrap(),
    ];
...
async fn caches_getaddr_response() {
    ...
    let Ok(zakura_network::Response::Peers(first_result)) = inbound
        .clone()
        .oneshot(zakura_network::Request::Peers)
        .await
    else {
    ...
    for _ in 0..NUM_REQUESTS {
        let Ok(zakura_network::Response::Peers(peers)) = inbound
            .clone()
            .oneshot(zakura_network::Request::Peers)
            .await
        else {
