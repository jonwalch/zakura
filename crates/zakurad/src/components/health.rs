//! HTTP health and readiness endpoints for `zakurad`.
//!
//! Overview
//!
    ...
    SyncStatus: ChainSyncStatus + Clone + Send + Sync + 'static,
{
    ...
    let listener = tokio::net::TcpListener::bind(listen_addr)
            .await
            .unwrap_or_else(|e| panic!("Opening health endpoint listener {listen_addr:?} failed: {e:?}. Hint: Check if another zakurad is running, or change the health listen_addr in the config."));
