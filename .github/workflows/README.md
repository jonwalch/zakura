...
- **`create-release.yml`** — the only supported path for creating `v*` release tags. Before a normal release, it requires the Mainnet VCT handoff canary and calls `release-binaries.yml` to build and verify every asset; then a protected environment lets the release GitHub App publish the draft and create the immutable tag. Emergency source-first releases skip both pre-tag gates. See the release runbook before using it.
