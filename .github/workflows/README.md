...
- **`create-release.yml`** — the only supported path for creating `v*` release tags. For a normal release, it dispatches the advisory Mainnet VCT handoff canary and calls `release-binaries.yml` to build and verify every asset; then a protected environment lets the release GitHub App publish the draft and create the immutable tag without waiting for the canary. Emergency source-first releases skip both pre-tag tasks. See the release runbook before using it.
